...
const checkAndAwardBadges = async (req, res) => {
  try {
    ...
    const contributorStats = await BadgeService.getContributorStats(repositoryId, userId);
    ...
    const existingBadgeIds = new Set(existingBadges.map(ub => ub.badge._id.toString()));
    ...
    for (const badge of badges) {
      ...
      const isEligible = await BadgeService.checkBadgeEligibility(contributor, badge);
    ...
    return res
      .status(500)
      .json(new ApiResponse(500, "Failed to check badges"));
  }
};
      ...
      .json(
        new ApiResponse(200, "User badges retrieved", {
          ...
          summary: {
            ...
            progressPercentage: Math.round((userBadges.length / allBadges.length) * 100),
          },
        })
      );
  } catch (error) {
